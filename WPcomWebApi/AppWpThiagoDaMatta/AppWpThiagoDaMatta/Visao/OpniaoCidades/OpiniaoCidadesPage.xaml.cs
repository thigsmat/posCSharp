...
namespace AppWpThiagoDaMatta.Visao.OpniaoCidades
{
    ...
    public sealed partial class OpiniaoCidadesPage : Page
    {
        ...
        {
            InitializeComponent();
            Loaded += ReadOpiniaoList_Loaded;
        }

        private async void ReadOpiniaoList_Loaded(object sender, RoutedEventArgs e)
        {
            if (Cidade != null)
            {
                DB_OpiniaoList = await new OpiniaoCidadesControlador().ListaTodosPorCidade(Cidade.Id);
        ...
        private void AdicionarOpiniaoButton_Click(object sender,
          RoutedEventArgs e)
        {
